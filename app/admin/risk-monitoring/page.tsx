...
export default function RiskMonitoringPage() {
  ...
  const [properties, setProperties] = useState(RISK_PROPERTIES)

  useEffect(() => {
    async function loadRiskProperties() {
      try {
        const res = await get<{ data: any[] }>('/api/v1/properties/search?limit=20')
        const serverProps = res.data?.data
        if (Array.isArray(serverProps) && serverProps.length > 0) {
          const liveRiskItems = serverProps
            .filter((p: any) => typeof p.fraudRiskScore === 'number' || (p.redFlags && p.redFlags.length > 0))
            .map((p: any) => {
              const score = typeof p.fraudRiskScore === 'number'
                ? Math.round(p.fraudRiskScore > 1 ? p.fraudRiskScore : p.fraudRiskScore * 100)
                : 35
              const level: 'High Risk' | 'Medium Risk' | 'Low Risk' =
                score >= 70 ? 'High Risk' : score >= 45 ? 'Medium Risk' : 'Low Risk'
              return {
                id: p._id || p.id,
                title: p.title || 'Untitled Property',
                landlord: p.landlordId?.name || p.landlordId?.legalName || 'Landlord',
                riskScore: score,
                reasons: Array.isArray(p.redFlags) && p.redFlags.length > 0
                  ? p.redFlags
                  : ['Tabular anomaly check passed', 'Price within normal range'],
                status: level
              }
            })

          if (liveRiskItems.length > 0) {
            setProperties([...liveRiskItems, ...RISK_PROPERTIES])
          }
        }
      } catch (err) {
        console.warn('Could not load live risk properties from backend:', err)
      }
    }
  ...
  const active = properties.find((p) => p.id === selectedProperty) ?? null
