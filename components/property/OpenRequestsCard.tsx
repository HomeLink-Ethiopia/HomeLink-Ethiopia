import type { MaintenanceRequestSummary } from '@/lib/propertyDetails'

const STATUS_STYLE: Record<MaintenanceRequestSummary['status'], string> = {
...
const ICON_FOR_REQUEST: Record<string, JSX.Element> = {
  ...
  'Light bulb replacement': (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4">
      <path d="M9 18h6M10 22h4M15 8a5 5 0 1 0-6 4.9V17h6v-4.1A5 5 0 0 0 15 8z" />
    </svg>
  ),
}
...
export default function OpenRequestsCard({ requests }: { requests: MaintenanceRequestSummary[] }) {
  return (
    <div className="rounded-lg border border-charcoal/10 bg-white p-6">
      <div className="flex items-center justify-between">
        ...
        {requests.length > 0 && (
          <button type="button" className="text-sm font-medium text-rust hover:text-rust-dark transition-colors">
            View all
          </button>
        )}
      </div>
      ...
      {requests.length === 0 ? (
        <p className="mt-4 text-sm text-charcoal/50">No open maintenance requests on this property.</p>
      ) : (
